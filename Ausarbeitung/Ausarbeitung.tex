...
\begin{document}
	...
	\begin{proof}
		Sei $n' := \sum_v q_v = \left|V'\right|$ und $m := \frac{n'}{Q}$. Dann gilt für die Summe aller möglichen Routen:
			\[\sum_{v,i} d'(\sigma_i^{(v)}) = 2m\sum_{v'\in V'}d'(s,v') + (n'-m)d(\tau)\]
		Denn jeder Knoten ist genau einmal Anfangs- und einmal Endpunkt für jede der $m$ Teilstrecken (erster Summand) und jede Route entsteht durch Weglassen von $m$ Kanten aus der Tour $\tau$ (zweiter Summand).
		
		Die beste dieser Routen ist sicher mindestens so gut wie der Durchschnitt aus allen möglichen Routen, also:
			\[\sum_{i} d'(\sigma_i) = 2\frac{m}{n'}\sum_{v'\in V'}d'(s,v') + (1-\frac{m}{n'})d(\tau) \leq 2\frac{m}{n'}\sum_{v'\in V'}d'(s,v') + (1-\frac{m}{n'})\rho\cdot\OPT_\TSP\]
		Zusammen mit \Cref{CVRPLowerBound} ergibt dies:
			\[\sum_{i} d'(\sigma_i) \leq \frac{mQ}{n'}\OPT_\CVRP + (1-\frac{m}{n'})\rho\cdot\OPT_\CVRP \leq \left(1+\frac{Q}{n'}+\rho\right)\cdot\OPT_\CVRP\]
	\end{proof}
	...
	\begin{algorithm}[H]
		...
		\begin{algorithmic}[1]
			...
			\State $L := \{T \in \Tc\},\quad b(T) := d(T)$, \quad $R := \{i \mid 1\leq i \leq k\},\quad b(i) := \beta 2^{\lambda_i}$
			\State $F := \{\{T,i\} \mid T \in \Tc_l, \lambda_i \geq l-1\}$ 
			\State Bestimme rationales, $L$-überdeckendes $b$-Matching $x_{Ti}$.
